...
parser.add_argument('--valDataroot', required=False,
  default='./facades/test416_more', help='path to val dataset')
...
parser.add_argument('--netG', default='./DUAN/netG_144.pth', help="path to netG (to continue training)")
...
#    vutils.save_image(dehaze, './compare_new/DADN_256_baseline_vgg_gan/'+str(index)+'.png', normalize=False,scale_each=False) 
    dehaze_test = dehaze.squeeze(0).mul_(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to('cpu', torch.uint8).numpy()
    test_free=data_free.squeeze(0).mul_(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to('cpu', torch.uint8).numpy()
    dehaze_free = defree.squeeze(0).mul_(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to('cpu', torch.uint8).numpy()    
...
#    vutils.save_image(defree, './compare_new/DADN_256_baseline_vgg_gan_gt/'+str(index)+'.png', normalize=False,scale_each=False)
    index = index+1
    print(index)
print('input is hazy: psnr='+str(psnr_count/len(dataloaderhaze))+' ssim=' + str(ssim_count/len(dataloaderhaze)))
print('input is hazy-free: psnr='+str(psnr_count_free/len(dataloaderhaze))+' ssim=' + str(ssim_count_free/len(dataloaderhaze)))
