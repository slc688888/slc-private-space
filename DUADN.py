...
parser.add_argument('--dataroot', required=False,
  default='./facades/train416_more', help='path to trn dataset')
...
parser.add_argument('--exp', default='DUAN_multiple', help='folder to output images and model checkpoints')
...
#            haze_train1=torch.flip(haze_train1,[-1])
#            free_train1=torch.flip(free_train1,[-1])
#        haze_train1 = haze_train1.squeeze(0).mul_(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to('cpu', torch.uint8).numpy()
#        free_train1 = free_train1.squeeze(0).mul_(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to('cpu', torch.uint8).numpy()
#        im2=Image.fromarray(free_train1)
#        im=Image.fromarray(haze_train1)
#        im.show()
#        im.save('free.png')
...
#                        ])
#        haze_train1=tran(im).unsqueeze(0)
#        free_train1=tran(im2).unsqueeze(0)
        
        ''' edge '''
#        edge_free = misc.edge_compute(free_train1)
#        edge_hazy = misc.edge_compute(haze_train1)
#        haze_train = torch.cat((edge_hazy,haze_train1),dim=1)
#        free_train = torch.cat((edge_free,free_train1),dim=1)
...
#        errLp_A_1 =  criterionMSE(features_A[0], f_A_1)
        errLp_A =  criterionMSE(features_A[1], f_A_2)
...
#        errLp_A =  0.5*errLp_A_1 + 0.1*errLp_A_2
        ...
        '''G loss'''
        errG = errc_A_P + 0.1*errLp_A + 0.1*errGan_A + erre + errc_A_F
        errG.backward()
        optimizerG.step()
...
#        test_hazy = test_hazy.squeeze(0).mul_(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).to('cpu', torch.uint8).numpy()
#        im3=Image.fromarray(test_hazy)
        with torch.no_grad():
    ...
    with open('results.txt', 'a') as f:
        f.write('epoch'+str(epoch)+'\t'+'psnr='+str(psnr_count/len(dataloadertest))+','+str(psnr_count_free/len(dataloadertest))+'\t'+'ssim='+str(ssim_count/len(dataloadertest))+','+str(ssim_count_free/len(dataloadertest)) + '\n')  # P, R, mAP, F1, test_losses=(GIoU, obj, cls)          
    schedulerD.step()
    schedulerG.step()
trainLogger.close()
