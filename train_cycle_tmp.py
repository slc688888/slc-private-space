...
parser.add_argument('--exp', default='samplecycle', help='folder to output images and model checkpoints')
...
#建立生成网络netG和判别网络netD
netG_A = net.UnetGenerator(inputChannelSize, outputChannelSize,8,ngf)
netG_B = net.UnetGenerator(inputChannelSize, outputChannelSize,8,ngf)
...
#        errG = errGan_A + errGan_B +err_cycle_A+ err_cycle_B+errc_A+errc_B+0.3*errLp_A+0.3*errLp_B
        errG = errGan_A + errGan_B +err_cycle_A+ err_cycle_B+0.01*errLp_A+0.01*errLp_B+errc_A+errc_B
        errG.backward()
        optimizerG.step()
        '''netD'''
        for p in netD_A.parameters():
        ...
        label_A = torch.FloatTensor(output_A_1.size()).fill_(real_label).cuda()
        errD_A_real = criterionMSE(output_A_1,label_A)      
        fake_free = netG_A(haze_train)
        output_A_2 = netD_A(fake_free)
        label2 = torch.FloatTensor(output_A_2.size()).fill_(fake_label).cuda()
        errD_A_fake = criterionMSE(output_A_2,label2)       
        errD_A = (errD_A_real + errD_A_fake)
        '''netD_B'''
        output_B_1 = netD_B(haze_train)
        label_B = torch.FloatTensor(output_B_1.size()).fill_(real_label).cuda()
        errD_B_real = criterionMSE(output_B_1,label_B)    
        fake_haze = netG_B(free_train)
        output_B_2 = netD_B(fake_haze)
        label2 = torch.FloatTensor(output_B_2.size()).fill_(fake_label).cuda()
        errD_B_fake = criterionMSE(output_B_2,label2)       
        ...
        if ganIterations % opt.display == 0:
            print('[%d/%d][%d/%d]\
                  L_G_A: %f L_G_B: %f L_GC_A: %f L_GC_B: %f Lc_A: %f Lc_B: %f \
                  L_D_A_R: %f L_D_A_F: %f L_D_B_R: %f L_D_B_F: %f\
                  L_G: %f L_D_A: %f L_D_B: %f'
...
#                 errG.data[0],errD_A.data[0],errD_B.data[0],errc_A.data[0],errc_B.data[0]))
              % (epoch, opt.niter, i, len(dataloaderhaze),
                 errGan_A.data[0], errGan_B.data[0],err_cycle_A.data[0],err_cycle_B.data[0],errLp_A.data[0],errLp_B.data[0],                
                 errD_A_real.data[0], errD_A_fake.data[0], errD_B_real.data[0], errD_B_fake.data[0],
                 errG.data[0],errD_A.data[0],errD_B.data[0]))
            sys.stdout.flush()
            trainLogger.write('%d\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n' % \
                            (i,errGan_A.data[0], errGan_B.data[0],err_cycle_A.data[0],err_cycle_B.data[0],errLp_A.data[0],errLp_B.data[0],                
                 errD_A_real.data[0], errD_A_fake.data[0], errD_B_real.data[0], errD_B_fake.data[0],
                 errG.data[0],errD_A.data[0],errD_B.data[0],errLp_A.data[0],errLp_B.data[0]))
            trainLogger.flush()
